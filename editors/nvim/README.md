...
Filetype detection, tree-sitter highlighting and folding, auto-indent and the `novn lsp` language server for Neovim 0.11+.
...
- The `novn` binary on `$PATH`: `cargo install --path crates/novn-cli`.
...
| `lsp/vn.lua` | Config for `vim.lsp.enable("vn")`: runs `novn lsp`, rooted at the nearest `schema.json`. |
