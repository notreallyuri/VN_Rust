...
- [x] Optional native video cutscenes, before M9 Extras: `video` selects rav1d + Matroska demuxing + Vorbis; `video-ffmpeg` selects FFmpeg, including when both features are enabled. Neither is enabled by default
  ...
  - [ ] Release validation on Windows/macOS and lower-end hardware; measure real footage, sustained sync, CPU/memory and shipped size. The earlier conversation's synthetic decoder-only 66/250/341 fps figures are not end-to-end acceptance results. FFmpeg currently requires externally supplied matching development/runtime libraries; automated trimmed LGPL builds and distribution remain separate work
