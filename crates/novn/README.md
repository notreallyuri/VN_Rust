...
A raylib-based visual novel engine built on [`novn_script`](../novn-script/README.md).

It re-exports `raylib` and `novn_script` (as `novn::script`), so games build against
the same versions the engine uses and only need `novn` as a dependency (plus
[`novn_build`](../novn-build/README.md) as a build dependency to [ship release builds](#shipping-a-release-build)).
...
Every item can be reached at its own path — `novn::ui::button::ButtonStyle`,
`novn::data::saves::Saves` — and each group also has a `prelude` with the everyday
...
`novn::prelude` is the short list that nearly every game file needs: `Action`,
...
`cargo run -p novn --example corner_shapes` shows every shape with a border, a
...
(`assets/lang/<code>.json`, written by `novn translate` — see
[novn_script's README](../novn-script/README.md#translation)). It registers what it has:
...
`novn translate <lang>` folds that list into the catalog. A game can add strings it builds
...
`#[command]` (re-exported from [`novn_macros`](../novn-macros/README.md), and in the prelude)
...
`#[derive(StoryWord)]` (also re-exported from [`novn_macros`](../novn-macros/README.md)) makes
...
**`novn check` catches a word that is not in the set before the game runs**, the way it
...
In `novn_script`, scene entries are `Event::SceneEnter { scene }` events, off by default
...
The registries only exist in Rust, so tools that don't run the game (`novn check`, the
LSP) read them from `<assets>/schema.json` (a `novn_script::SchemaFile`: title, story
...
`StoryVm::snapshot()` / `restore()` in `novn_script`.
...
```sh
cargo run -p novn --example video --features video
cargo run -p novn --example video --features video-ffmpeg -- vp9-opus.webm
```
...
[`novn_live2d`](../novn-live2d/README.md).
...
```sh
cargo run -p novn --features character-visuals --example puppet
```
...
`cargo run -p novn --features character-visuals --example puppet` plays through one:
...
instance its place. `novn_live2d` is the worked example of a backend that owns GPU state;
...
anywhere, embed the folder with [`novn_build`](../novn-build/README.md) from `build.rs` and
pass the result:
...
`novn_build` only embeds files in release builds (debug builds get an empty list, so they
...
the names they hold, so `novn::ButtonStyle`, `novn::ui::draw_text`,
`novn::post::GRAIN` and `novn::saves::SaveFile` all resolve whatever folder the
...
A few checks that need a GPU are `#[ignore]`d and run with `cargo test -p novn -- --ignored`.
