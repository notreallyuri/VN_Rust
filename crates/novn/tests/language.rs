...
use novn::game::language::{Language, load_catalog};
use novn::screens::settings::SettingsConfig;
use novn::screens::settings::SettingsRow;
use novn::script::translate;
use novn::script::translate::Catalog;
...
fn a_hot_reload_keeps_the_language() {
    ...
    use novn::game::hot_reload::swap_story;
    use novn::script::{StoryVm, compile_sources};
...
fn a_game_that_renames_a_label_offers_the_new_name() {
    let app = VnApp::new("Test").main_menu(|m| {
        m.button("Begin the Archive", novn::action::Action::NewGame)
            .button("Leave", novn::action::Action::Quit)
    });
    let strings = app.ui_strings().strings;
...
fn the_glyphs_a_font_needs_come_from_the_story_and_every_catalog() {
    use novn::game::language::charset;
    use novn::script::compile_sources;
