...
use novn::input::image_map::{Hotspot, ImageMap, ImageMapStyle};
...
fn hotspots_follow_the_picture_when_the_window_does_not_match_it() {
    ...
    let same = novn::ui::cover_rect(picture, area());
    assert_eq!(desk.bounds(same).x, 320.0);

    let wide = novn::ui::cover_rect(picture, Rectangle::new(0.0, 0.0, 1600.0, 720.0));
