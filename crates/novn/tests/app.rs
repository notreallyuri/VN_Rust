...
use novn::app::{AppError, VnApp};
use novn::context::GameContext;
use novn::game::characters::{Character, Characters};
use novn::game::commands::FromArgs;
use novn::screen::ScreenState;
use novn::script::{CommandSig, ParamKind, Severity, StoryVm};
use novn::script::{Value, VariableDef};
...
fn every_story_file_is_loaded() {
    ...
    while let novn::script::Event::Say { text, .. } = story.advance_until_blocking() {
        lines.push(text);
    }
...
fn schema_export_describes_the_game() {
    ...
    assert_eq!(novn::script::SchemaFile::read(&path).unwrap(), file);
