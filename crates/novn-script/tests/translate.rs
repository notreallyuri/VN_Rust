use novn_script::translate::{self, Catalog, Entry, Source, StringKind};
use novn_script::{CharacterDef, Event, Schema, StoryVm, compile_sources};
...
fn program(source: &str) -> novn_script::Program {
...
fn schema() -> Schema {
    ...
    schema
        .variables
        .insert("read".into(), novn_script::VariableDef::bool(false));
...
fn a_translated_line_still_gets_its_variables() {
    ...
    vm.set_catalog(Some(catalog));
    vm.set_variable("name", novn_script::Value::String("Hugo".into()))
        .unwrap();
...
fn changing_the_catalog_retranslates_the_choice_on_screen() {
    ...
    assert_eq!(
        vm.variable("read"),
        Some(&novn_script::Value::Bool(true)),
...
fn ui_strings_round_trip_through_a_file() {
    let strings = novn_script::UiStrings::new(
        ["Back", "Settings", "Back", ""]
            .into_iter()
            .map(String::from),
    );
    ...
    assert_eq!(novn_script::UiStrings::read(&path).unwrap(), strings);
    let _ = std::fs::remove_file(&path);
}
