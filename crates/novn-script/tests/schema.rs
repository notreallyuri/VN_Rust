...
fn schema_files_round_trip() {
    let mut file = novn_script::SchemaFile::new("Game", "story", schema());
    file.entry_scene = Some("start".into());

    let json = file.to_json();
    assert_eq!(novn_script::SchemaFile::from_json(&json), Ok(file.clone()));
    ...
    assert!(!file.write(&path).unwrap());
    assert_eq!(novn_script::SchemaFile::read(&path).unwrap(), file);
    std::fs::remove_file(&path).unwrap();
}

#[test]
fn schema_files_can_leave_registries_out() {
    let file = novn_script::SchemaFile::from_json(
...
fn newer_schema_files_are_rejected() {
    let error = novn_script::SchemaFile::from_json(
...
fn a_story_calling_a_command_with_an_unknown_word_is_an_error() {
    ...
    let program = novn_script::compile_source("scene start:\n  call close_case keper\n");
