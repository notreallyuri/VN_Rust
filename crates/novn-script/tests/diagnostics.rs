use novn_script::{Node, Severity, Value, compile_source, parse_program, tokenize};
...
fn narration_ending_in_a_colon_is_narration() {
    ...
    assert_eq!(tokens[1].kind, novn_script::TokenKind::Narration);
    assert_eq!(tokens[2].kind, novn_script::TokenKind::ChoiceOption);
}
...
fn string_escapes() {
    ...
    let texts: Vec<String> = program
        ...
        .filter_map(|i| match i {
            novn_script::Instruction::Say { text, .. } => Some(text.clone()),
            novn_script::Instruction::Set { value, .. } => Some(value.to_string()),
            novn_script::Instruction::Choice { options, .. } => Some(options[0].text.clone()),
            _ => None,
        })
        .collect();
...
fn escapes_in_conditions() {
    assert_eq!(
        novn_script::parse_condition(r#"name == "a \"b\"""#, 1),
        Ok(novn_script::Condition::Test {
            var_id: "name".into(),
            op: novn_script::Comparison::Equal,
...
fn diagnostics_carry_their_file() {
    let program = novn_script::compile_sources([
        ("a.story", "scene a:\n  show mary\n"),
        (
            "b.story",
            "scene b:\n  \"ok\"\nscene a:\n  \"dup\"\n  jump nowhere\n",
    ...
    let shown: Vec<String> = novn_script::Schema::default()
        .validate(&program)
        .iter()
...
fn same_file_duplicates_name_the_line() {
    let program =
        novn_script::compile_sources([("a.story", "scene a:\n  \"x\"\nscene a:\n  \"y\"\n")]);
    assert_eq!(
...
fn a_reason_is_told_apart_from_a_string_compared_in_the_condition() {
    ...
    let novn_script::Instruction::Choice { options, .. } = &program.instructions[0] else {
...
fn a_choice_where_every_option_can_be_hidden_is_a_warning() {
    ...
    let schema = novn_script::Schema::default();
