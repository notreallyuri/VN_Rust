...
the engine, the `novn` CLI and future tooling (LSP, formatter) can all share it.
...
per instruction and a header per scene (`scene start:  (file:line)`). `novn dump
...
interpolation uses. `Value::literal()` gives the DSL form (`"Yuri"`), which `novn dump` uses.
...
`Condition` implements `Display`, which `novn dump` uses.
...
- The compiled program keeps the text as written (`novn dump` shows `{player_name}`).
...
so tools can validate stories without running the game (`novn check`, and later the LSP):
...
`novn translate <lang>` command writes the catalog; the engine loads one and hands it to the
VM.
...
`novn translate --export/--import` carries both to and from a `.po` file (see
[novn_cli's README](../novn-cli/README.md)), which is how the two flags reach a translation
tool as `#~` and `#, fuzzy`.
...
| `UiStrings` | The list of labels a frontend offers for translation (`lang/ui.json`), which `novn translate` folds into the `ui` section |
...
| `prepare(schema, entry_scene) -> Vec<Diagnostic>` | `set_schema` + `set_entry_scene` + `validate` in one call, with an error for an unknown entry scene. What the engine and `novn check` run |
