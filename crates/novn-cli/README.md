...
The `novn` command-line tool for working with `.story` files.
...
suggestion (see novn_script's README, "Schema and validation").

The schema is the `schema.json` a game exports (see novn's README, "Schema export").
...
Without `--schema`, `novn check` looks for `schema.json` in `path`'s directory and each of
its parents. With no schema anywhere it checks syntax and jump targets only, and says so.

#### Translations

When the project has a `lang/` directory, `novn check` ends with a line per catalog,
counted against the story as it is now rather than as it was at the last `novn translate`:
...
```sh
cargo run -p novn-cli -- fmt examples/god_is_watching/assets/story --check
```
...
```sh
cargo run -p novn-cli -- translate pt-BR examples/god_is_watching/assets
```
...
Without that file `novn translate` says so and extracts the story only.

`path` is the project (a `schema.json`, or any directory at or under it — the same search
as `novn check`); it defaults to the working directory. A story with errors is reported and
...
[novn_script's README](../novn-script/README.md#translation).
...
```sh
cargo run -p novn-cli -- translate pt-BR examples/god_is_watching/assets --export
```
...
```sh
cargo run -p novn-cli -- translate pt-BR examples/god_is_watching/assets --import lang/pt-BR.po
```
...
| File | Contents |
| --- | --- |
| `Cargo.toml` | A package named after the directory (`my-novel` → `my_novel`), depending on `novn` (and `novn_build` for the build script, from the same place), with an empty `[workspace]` so it builds on its own even inside another workspace |
| `src/main.rs` | A `VnApp` with one character (`guide`), two variables and a `give_item` command, and the embedded assets |
| `build.rs` | Embeds `assets/` in release builds with `novn_build`, so `cargo build --release` gives an executable that runs anywhere |
...
parent directory), `--engine-git`, or by default the `novn` next to the `novn_cli` that
...
Install the binary once (`cargo install --path crates/novn-cli`), then point the editor at
`novn lsp` for `*.story` files.
...
The project is found like `novn check` does: the nearest `schema.json` above the file, and
...
`tests/check.rs` runs the `novn` binary on temporary projects: valid projects from each
...
`--schema`, unreadable inputs, and `dump` on a directory. `tests/new.rs` runs `novn new`:
the files it writes pass `novn check`, the package name and engine path, `--title` and
`--engine-git` (with quotes in the title), non-empty directories, names that aren't crate
names, and bad arguments.
`tests/translate.rs` runs `novn translate`: the catalog it writes and where, a second run
...
`tests/lsp.rs` runs `novn lsp` over pipes like an editor: diagnostics following unsaved
