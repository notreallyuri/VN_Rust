...
fn cargo_toml_names_the_package_and_points_at_the_engine() {
    ...
    let path = cargo
        .lines()
        .find_map(|line| line.strip_prefix("novn = { path = \""))
...
fn title_and_engine_git_flags() {
    ...
    assert!(
        temp_project
            .read("Cargo.toml")
            .contains("novn = { git = \"https://example.com/vn.git\" }")
    );
    assert!(
        temp_project
            .read("Cargo.toml")
            .contains("novn-build = { git = \"https://example.com/vn.git\" }")
    );
    assert!(
        temp_project
            .read("src/main.rs")
