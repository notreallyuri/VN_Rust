...
use novn_script::{CharacterDef, CommandSig, ParamKind, Schema, SchemaFile, VariableDef};
...
impl Client {
    fn start() -> Self {
        let mut child = Command::new(env!("CARGO_BIN_EXE_novn"))
            .arg("lsp")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .spawn()
        ...
        assert_eq!(init["serverInfo"]["name"], "novn");
