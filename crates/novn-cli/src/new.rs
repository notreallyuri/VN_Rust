...
pub fn create(options: &Options) -> Result<Created, String> {
    ...
    let build = engine.replace("novn", "novn-build");
...
fn engine_dependency(engine: Option<&Engine>, project: &Path) -> Result<String, String> {
    let bundled = Path::new(env!("CARGO_MANIFEST_DIR")).join("../novn");
    ...
    match engine {
        ...
        Engine::Path(path) => {
            ...
            if !engine
                .with_file_name("novn-build")
                .join("Cargo.toml")
                .is_file()
            {
                return Err(format!(
                    "{} has no novn-build crate next to it",
                    engine.display()
                ));
            }
...
const BUILD_RS: &str = r#"fn main() {
    novn_build::Embed::new("assets").exclude(["schema.json"]).run();
}
"#;
...
A visual novel made with [novn]({ENGINE_GIT}).
...
| `assets/schema.json` | The registries, rewritten by debug builds, for `novn check` |
