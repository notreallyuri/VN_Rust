...
use novn_script::{Diagnostic, SCHEMA_FILE_NAME, SchemaFile, StoryVm, compile_sources};
...
fn report_translations(
    ...
    for code in &codes {
        ...
        if lang.is_none() && !status.is_complete() {
            println!("  run `novn check --lang {}` to list them", status.language);
        }
    }
