...
enum into the fixed set of words such a command accepts. `vn_engine` re-exports both (and
...
[vn_engine's Commands](../vn_engine/README.md#commands) for the types a parameter may
have.
...
which goes into `schema.json`, so `vn check` rejects a word outside the set (with a
...
Both macros are exercised from `vn_engine`, where the engine side of them lives:
`crates/vn_engine/tests/commands.rs` (names, renaming, the signature flat parameters
