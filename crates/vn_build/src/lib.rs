...
impl Embed {
    ...
    pub fn run(self) {
        ...
        let out = PathBuf::from(env::var_os("OUT_DIR").expect("vn_build runs from a build script"));
        ...
        let files = if embed {
            self.files(&dir).unwrap_or_else(|e| {
                panic!(
                    "vn_build: could not read the assets in {}: {}",
                    dir.display(),
                    e
                )
            })
        ...
        if fs::read_to_string(&path).ok().as_deref() != Some(source.as_str()) {
            fs::write(&path, source).expect("vn_build: could not write the embedded asset list");
        }
    }
