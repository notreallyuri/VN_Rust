...
fn expand(alias: Option<LitStr>, function: ItemFn) -> Result<proc_macro2::TokenStream, Error> {
    ...
    Ok(quote! {
        ...
        impl ::novn::game::commands::Command for #marker {
            type Args = #args;
            const NAME: &'static str = #name;

            fn run(context: &mut ::novn::context::GameContext, arguments: Self::Args) -> #output {
                #taken
...
fn expand_word(input: syn::DeriveInput) -> Result<proc_macro2::TokenStream, Error> {
    ...
    Ok(quote! {
        ...
        impl ::novn::game::commands::FromArg for #name {
            fn kind() -> ::novn::script::ParamKind {
                ::novn::script::ParamKind::Choice(Self::words())
            }
        ...
        impl ::novn::game::commands::Arg for #name {
            const OPTIONAL: bool = false;

            fn kind() -> ::novn::script::ParamKind {
                <Self as ::novn::game::commands::FromArg>::kind()
            }
        ...
        impl ::novn::serde::Serialize for #name {
            fn serialize<S: ::novn::serde::Serializer>(
                &self,
                serializer: S,
        ...
        impl<'de> ::novn::serde::Deserialize<'de> for #name {
            fn deserialize<D: ::novn::serde::Deserializer<'de>>(
                deserializer: D,
            ) -> ::core::result::Result<Self, D::Error> {
                let word = <::std::string::String as ::novn::serde::Deserialize>::deserialize(
                    deserializer,
                )?;
                Self::from_word(&word).ok_or_else(|| {
                    <D::Error as ::novn::serde::de::Error>::custom(::std::format!(
                        #unknown,
                        word
                    ))
                })
