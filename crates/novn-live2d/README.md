...
The dependency runs one way: `novn_live2d` uses `novn`, never the reverse. The
seam lives behind `novn`'s `character-visuals` feature, which is off by
default, and this crate's `engine` feature turns it on. That seam now carries a second
backend, the engine's own [puppet](../novn/README.md#character-visuals) — flat parts
...
- The engine seam compiles, lints and tests with `--features character-visuals`;
  ...
  `cargo test -p novn --features character-visuals`, which now includes
...
- The seam carries a second backend: `novn`'s puppet. Its probe was watched through
...
4. ~~Save logical appearance/parameter state and restore it on load and rollback.~~
   ...
   down in [the engine's README](../novn/README.md#while-the-story-is-not-playing);
