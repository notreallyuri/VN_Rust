...
fn main() -> Result<(), novn::app::AppError> {
    ...
    let settings: novn::serde_json::Value =
        novn::serde_json::from_slice(&fs::read(model_dir.join(&model)).unwrap()).unwrap();
    ...
    let parameters: Vec<String> = references["DisplayInfo"]
        ...
        .and_then(|bytes| novn::serde_json::from_slice::<novn::serde_json::Value>(&bytes).ok())
        .and_then(|info| {
    ...
    if broken {
        static FALLBACK: &[u8] = include_bytes!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/../novn/tests/fixtures/puppet/characters/mary/body.png"
        ));
    ...
    VnApp::new("Live2D story")
        ...
        .shader("grain", novn::frame::post::GRAIN)
        .shader("desaturate", novn::frame::post::DESATURATE)
